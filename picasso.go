...
var palette [128]string = [128]string{
...
func channelFromHexChunk(hex string) uint8 {
	channel, err := strconv.ParseInt(hex, 16, 64)

	if err != nil {
		fmt.Println("Hex value out of bounds")
		return 0
	}
...
func hexToRGB(hex string) (uint8, uint8, uint8) {
	...
	for i := range rgb {
		rgb[i] = channelFromHexChunk(hex[i*2 : (i*2)+2])
	}
